...
Description:
------------
This project detects smoke-emitting vehicles from traffic
videos using YOLO-based object detection, vehicle tracking,
license plate recognition (OCR), and automatic evidence
generation.
...
smoke_model = YOLO(os.path.join(MODEL_DIR, "best.pt"))

vehicle_model = YOLO(os.path.join(MODEL_DIR, "yolov8n.pt"))

plate_model = YOLO(os.path.join(MODEL_DIR, "plate.pt"))
reader = easyocr.Reader(["en"])

# =====================================
# INPUT VIDEO
# =====================================
VIDEO_PATH = os.path.join(BASE_DIR, "input", "input.mp4")
# =====================================
# OUTPUT FOLDERS
# =====================================
...
while True:
    ...
    for stable_id, vehicle_box, name in current_vehicles:
        ...
        suspect = (
            checked >= MIN_TRACK_HISTORY
            and smoke_count >= MIN_SMOKE_FRAMES
            and track_age[stable_id] >= MIN_TRACK_AGE
        )
                ...
                if vehicle_crop is not None and vehicle_crop.size > 0:
                    try:
                        plate_results = plate_model(
                            vehicle_crop, conf=PLATE_CONFIDENCE, verbose=False
                        )
...
print("=" * 50)
print(f"Total Suspect Vehicles : {len(saved_suspects)}")
